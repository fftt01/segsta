...
class seg_dataset(Dataset):
    def __init__(self, inner_dataset, train_cur):
        # 打开 HDF5 文件
        self.inner_dataset = inner_dataset

        self.train_cur = train_cur
        self.select_range = 12 // train_cur
        self.mean = inner_dataset.mean
        self.std = inner_dataset.std

    def __len__(self):
        return self.inner_dataset.__len__()

    def __getitem__(self, idx):
        data, labels = self.inner_dataset.__getitem__(idx)
        random_int = torch.randint(0, self.train_cur, (1,)).item()
        return random_int, data, labels[random_int*self.select_range:(random_int+1)*self.select_range]
...
def load_data(batch_size, val_batch_size, data_root, num_workers=4,
              ...
              distributed=False, use_augment=False, use_prefetcher=False, drop_last=False, train_cur=None):
    ...
    if train_cur is not None:
        train_set = seg_dataset(train_set, train_cur)
        test_set = seg_dataset(test_set, 1)
